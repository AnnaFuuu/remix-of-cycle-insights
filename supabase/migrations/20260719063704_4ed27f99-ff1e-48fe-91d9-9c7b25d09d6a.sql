...
sleep_score_daily AS (
  ...
  WHERE day_in_study IS NOT NULL GROUP BY participant_id, day_in_study
),
rhr_daily AS (
...
wrist_temp_daily AS (
  SELECT participant_id, day_in_study,
    ...
    AVG((raw->>'overnight_mean')::numeric) AS wrist_temp_overnight_mean,
    SUM(COALESCE((raw->>'n_minutes')::numeric, 0)) AS wrist_temp_samples
  FROM public.mcphases_wrist_temperature
